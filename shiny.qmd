...
In this next more complicated app we'll create an interactive time series plot and map. Users can select a station from a drop-down menu and see the time series of for any of the available variables from another drop-down selection. The map will show the location of the selected station.
...
1. Let's assume we want three variables: Total Nitrogen (mg/L) (`tn`), Chlorophyll-a (ug/L) (`chla`), and Secchi depth (m) (`sd_m`). Do we need to pivot _**longer**_ (`?tidyr::pivot_longer`) or _**wider**_ (`?tidyr::pivot_longer`) to get each row as its own observation?
...
1. The `epcdata` is currently in _**wide**_ format, with each variable as its own column. We want to pivot it _**longer**_ so that each row is an observation of a single variable.
