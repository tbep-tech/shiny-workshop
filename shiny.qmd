...
1.  Shiny registers that the `input$bins` value has changed and re-runs any functions in server.R that use this value, which in this case is `output$distPlot` defined by:
...
1.  The updated `output$distPlot` from server.R is refreshed in the layout of ui.R by `plotOutput()` since the first argument of this function is `outputID` which has the value `"distPlot"`:
