...
Observable is a relatively new language that also allows dynamic features to be included in a Quarto document.  It is an entirely separate language outside of R, but provides excellent functionality that approximates that provided by a Shiny Server.  

First, import the data as you would in R, then define variables for Observable using `ojs_define()`.
