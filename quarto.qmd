...
Observable is a relatively new language that also allows dynamic features to be included in a Quarto document.  It is an entirely separate language outside of R that uses JavaScript and allows excellent functionality similar what is provided by a Shiny Server.  

First, import the data as you would in R, then define variables for Observable using `ojs_define()`.  Note that Observable chunks cannot be run interactively and only work when an entire Quarto document is rendered. 
