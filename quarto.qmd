...
Observable is a relatively new language that also allows dynamic features to be included in a Quarto document.  It is an entirely separate language outside of R that uses JavaScript and allows excellent functionality similar what is provided by a Shiny Server.  The following plot showing a historam of chlorophyll values by bay segment is created using the code below.    
